  ...
  return (
    <div className="pb-8">
      ...
      <div className="px-4 py-4 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(order as { deliverySlot?: string | null }).deliverySlot && (
              <div className="flex items-center gap-2 bg-violet-50 border border-violet-100 rounded-xl px-3.5 py-2">
                ...
                <p className="text-xs font-semibold text-violet-800">{(order as { deliverySlot?: string }).deliverySlot}</p>
              </div>
            )}
          </div>
        )}
        ...
        {(order as { deliveryInstructions?: string | null }).deliveryInstructions && (
          <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-100 rounded-2xl px-4 py-3">
            <span className="text-base shrink-0">📝</span>
            <div>
              <p className="text-[10px] font-bold text-amber-700 uppercase tracking-wide mb-0.5">Delivery note</p>
              <p className="text-xs text-amber-900 font-medium">{(order as { deliveryInstructions?: string }).deliveryInstructions}</p>
            </div>
          </div>
        )}
